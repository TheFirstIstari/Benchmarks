...
// 2. Branch prediction
static void bench_branch_prediction() {
    std::vector<int> data(BRANCH_N);
    for (int i = 0; i < BRANCH_N; i++) data[i] = rand() % 100;
    ...
    for (int i = 0; i < BRANCH_N; i++) {
        if (data[i] > 50) sum_unsorted += data[i];
    }
    uint64_t t1 = now_ns();
    double ms_unsorted = (t1 - t0) / 1e6;
    printf("cpu_branch_unsorted: %.2f ms\n", ms_unsorted);
    
    volatile long sum_sorted = 0;
    t0 = now_ns();
    for (int i = 0; i < BRANCH_N; i++) {
        if (sorted[i] > 50) sum_sorted += sorted[i];
    }
    t1 = now_ns();
    double ms_sorted = (t1 - t0) / 1e6;
    printf("cpu_branch_sorted: %.2f ms (%.1fx faster)\n", ms_sorted, ms_unsorted / ms_sorted);
    
    if (sum_sorted == 0 || sum_unsorted == 0) printf("");
}

// 3. Memory latency
static void bench_memory_latency() {
    std::vector<int> indices(POINTER_N);
...
#ifdef __ARM_NEON
#include <arm_neon.h>
static void bench_simd() {
    float* a = (float*)aligned_alloc(16, SIMD_N * sizeof(float));
    float* b = (float*)aligned_alloc(16, SIMD_N * sizeof(float));
    float* c = (float*)aligned_alloc(16, SIMD_N * sizeof(float));
...
#else
static void bench_simd() {
    std::vector<float> a(SIMD_N), b(SIMD_N), c(SIMD_N);
    for (int i = 0; i < SIMD_N; i++) { a[i] = i * 1.0f; b[i] = (i % 100) * 1.0f; }
...
static void bench_atomic() {
    ...
    if (atomic_val.load() == 0 || regular_val == 0) printf("");
}
